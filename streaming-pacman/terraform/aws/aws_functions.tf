...
resource "aws_lambda_function" "event_handler_function" {
  ...
  environment {
    variables = {
      //ORIGIN_ALLOWED = "https://${aws_cloudfront_distribution.pacman.domain_name}"
