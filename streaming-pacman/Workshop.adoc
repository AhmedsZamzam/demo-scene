...
The scoreboard can be visualized in real time by clicking on the *SCOREBOARD* link in the pacman game (top right corner). 
...
But first lets make sure the data is flowing into the Confluent. 

1. In Confluent UI go to the cluster created by the terraform script - should end with ``` -pacman-workshop ```
