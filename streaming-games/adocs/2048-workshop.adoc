...
----
[start=3]
3. Now look at the scoreboard, it should be populated
...
[start=4]
4. A query to the scoreboard is sent using the list retrieved with the first api call in the IN where clause: 
+
[source,sql]
----
select USER_KEY->USER, HIGHEST_SCORE, HIGHEST_LEVEL, TOTAL_LOSSES from STATS_PER_USER WHERE GAME_NAME='2048';
----
